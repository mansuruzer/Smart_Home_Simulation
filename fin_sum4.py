...
# ✅ ZAMAN KONTROL FONKSİYONU
def is_time_in_range(hour, start_time, end_time):
...
def calculate_baseline_load(hour, devices_df):
    """Temel yük hesaplama - GÜNCELLENDİ ✅"""
    total_load = 0
    for _, device in devices_df.iterrows():
        if device['category'] == CATEGORIES['CRITICAL']:
            total_load += device['power_kw']
        elif device['category'] in [CATEGORIES['SHIFTABLE'], CATEGORIES['OPTIONAL']]:
            # ✅ YENİ: Yardımcı fonksiyonu kullan
            if is_time_in_range(hour, device['start_earliest'], device['end_latest']):
                total_load += device['power_kw']
    return total_load
...
# 3. GELİŞMİŞ ENERJİ YÖNETİM ALGORİTMASI - TÜM HATALAR DÜZELTİLDİ ✅
def advanced_energy_management(hours, devices_df, pv_df, tou_df, params, season='summer'):
    ...
    # ✅ EV bağlantı zamanlarını JSON'dan al
    plug_in_time = params['ev']['plug_in_time']  # "18:00"
    plug_out_time = params['ev']['plug_out_time']  # "07:00"
    ...
    for hour in hours:
        ...
        # ✅ YENİ: Dinamik EV bağlantı kontrolü
        ev_plugged_in = is_time_in_range(hour, plug_in_time, plug_out_time)
        ...
        # ✅ DÜZELTİLDİ: Tutarlı AC setpoint mantığı
        if load_threshold_violation:
            # Sadece setpoint'i artır (konfor bandı dışına çıkabilir)
            new_setpoint = min(25.0, ac_setpoint + 0.5)
            ...
            # Konfor bandına yavaşça dön
            ac_setpoint = max(params['ac']['comfort_max_summer'], ac_setpoint - 0.1)
            savings = params['ac']['savings_percentage_per_degree']
            degrees_above = max(0, ac_setpoint - params['ac']['comfort_max_summer'])
            ac_power = max(1.5, params['ac']['nominal_power_kw'] * (1 - savings * degrees_above))
        ...
        # ✅ YENİ: Sabit değer kullanımı
        shiftable_devices = devices_df[devices_df['category'] == CATEGORIES['SHIFTABLE']]
...
# 4. GÖRSELLEŞTİRME - ŞEBEKE GRAFİĞİ DÜZELTİLDİ ✅
def create_final_plots(results, params, season='summer'):
    ...
    # 3. ŞEBEKE ETKİLEŞİMİ - TAMAMEN DÜZELTİLDİ ✅
    hours = results['hour']
    grid_import = results['grid_import']
    grid_export = results['grid_export']
    
    # Şebekeden çekilen (pozitif değerler)
    ax3.bar(hours, grid_import, alpha=0.7, label='Sebekeden Cekilen', color='red')
    
    # Şebekeye verilen (pozitif değerler) - DÜZELTME ✅
    if any(x > 0 for x in grid_export):
        ax3.bar(hours, grid_export, alpha=0.7, label='Sebekeye Verilen', color='green')
...
def generate_comprehensive_report(results, params, season='summer'):
    ...
    # ✅ DÜZELTİLDİ: Mevsime göre konfor ihlali kontrolü
    if season == 'summer':
        comfort_violations = sum(1 for temp in results['ac_setpoint'] 
                               if temp > params['ac']['comfort_max_summer'])
    else:
        comfort_violations = sum(1 for temp in results['ac_setpoint'] 
                               if temp < params['ac']['comfort_min_winter'])
    ...
    # ✅ Şebeke etkileşimi raporu
    total_export = sum(results['grid_export'])
    total_import = sum(results['grid_import'])
    print(f"   • Sebekeden Cekilen: {total_import:.1f} kWh")
    print(f"   • Sebekeye Verilen: {total_export:.1f} kWh")
    print(f"   • Net Sebeke Etkilesimi: {total_import - total_export:.1f} kWh")
...
print(f"   • Şebekeye Verilen Enerji: ✅ GRAFİKTE GÖRÜNÜYOR")
